...
$(function () {
	for (var i = 0; i < 16; i++) {
		info[i] = {talk:false,volume:50,play:false};
	}
	
	// 检查插件是否已经安装过
	if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
		showConfirm("系统提示", "您还未安装过插件，是否下载插件？", function(){
			window.top.location="../WebComponents.exe";
		});
		return;
	}
	
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin(/*572*/"100%", /*330*/"100%", {
        iWndowType: 1,
		cbSelWnd: function (xmlDoc) {
			...
			$(".slider").myslider("change", info[g_iWndIndex].volume);
			var talk = info[g_iWndIndex].talk;
			if (!talk) {
				$("#talkBtn").attr("title", "开启对讲");
				$("#talkBtn").removeClass("btn-on");
				$("#talkBtn").addClass("btn-off");
			} else {
				$("#talkBtn").attr("title", "关闭对讲");
				$("#talkBtn").removeClass("btn-off");
				$("#talkBtn").addClass("btn-on");
			}
	...
	if (-1 == WebVideoCtrl.I_CheckPluginVersion()) {
		/*showAlert("检测到新的插件版本，双击开发包目录里的WebComponents.exe升级！");
		return;*/
		showConfirm("系统提示", "您还未安装过插件，是否下载插件？", function(){
			window.top.location="../WebComponents.exe";
		});
...
function clickLogin() {
	...
	console.info(szIP + "," + szPort + "," + szUsername + ", " + szPassword);
...
function getWindowStatus(index) {
	return I_GetWindowStatus(index);
}
...
function checkWinCount() {
	var wndCount = getWndCount();
	var playCount = 0;
	var windStatus = new Array();
	for (var i = 0; i < wndCount; i ++) {
		var status = getWindowStatus(i);
		if (status) {
			if (status.iPlayStatus != 0){// 不是关闭状态
				playCount ++;
				windStatus[i] = true;
			} else {
				windStatus[i] = false;
	...
	if (playCount == wndCount) {// 全部窗口处于激活状态
		if (wndCount < 16) {
			changeWndNum(Math.sqrt(wndCount) + 1);
		}
	}
}
...
function clickStartRealPlay() {
	...
	var index = getFirstIndex();// 从播放列表中查询剩下的可用窗口
	var iRet = WebVideoCtrl.I_StartRealPlay(szIP, {
		iStreamType: iStreamType,
		iChannelID: iChannelID,
		bZeroChannel: bZeroChannel,
...
// 设置音量
function clickSetVolume(iVolume) {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
	...
	if (oWndInfo != null) {
		...
		if (0 == iRet) {
			info[g_iWndIndex].volume = iVolume;
			szInfo = "音量设置成功！";
		} else {
			szInfo = "音量设置失败！";
		}
...
function clickStartVoiceTalk() {
	...
	if (0 == iRet) {
		szInfo = "开始对讲成功！";
		info[g_iWndIndex].talk = true;
	} else {
		szInfo = "开始对讲失败！";
	}
	//console.info(szIP + " " + szInfo);
}
...
function stopAll() {
	...
	var options = document.getElementById("ip").options;
