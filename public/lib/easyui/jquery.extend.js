...
+function($){
	...
	$.extend($.fn.datetimebox.defaults.rules, {
		isBefore: {
			validator: function(value, param){
				return new Date(value.replaceAll('-','/')) < new Date($(param[0]).datetimebox('getValue').replaceAll('-','/'));
			},
			message: '{1}'
		},
		isAfter: {
	...
	$.fn.extend({
		 ...
		 * @param options
		 *  dict 字典
		 *  enableEmptyItem 是否增加一个“空”项目（表示全部,或空选择)，默认true
		 */
		comboboxFromDict: function (options) {
			var dictItems;
			if (options.dict) {
				if (options.dict.length)
					dictItems = options.dict;
				else if (typeof options.dict.items != 'undefined')
					dictItems = options.dict.items;
				else {
					dictItems = _.map(options.dict, function(v, k) { return {value: k, text: v} });
				}
			}
			if (options.emptyItem
				|| (typeof options.enableEmptyItem == 'undefined' || options.enableEmptyItem)) {
				var emptyItem = options.emptyItem || new DictItem("", "全部");
				options.data =  [emptyItem].concat(dictItems);
			} else {
				options.data = dictItems;
			}
