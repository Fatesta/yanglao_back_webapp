        ...
        <el-col :span="24">
          <el-form ref="userInfoForm" :model="form"
            ...
            <el-form-item
              ...
              label="用餐人数"
              :rules="[{required: true, message: ' '}]">
              <el-input-number v-model="form.mealNumber" controls-position="right" :min="1" />
            </el-form-item>
          </el-form>
        </el-col>
      </el-row>
      ...
      this.$refs.userInfoForm.validate(async (valid) => {
        ...
        let productListJSON = JSON.stringify(this.selections.map((sel) => {
          ...
          return {
              ...
              quantity: "1",
              productStatus: sel.product.status,
              imageFile: sel.product.imageFile,
              cateringIntegral: "1",
              orderDesc: form.remark,
          };
        }));
        let data = {
          ...
          sendIntegral: Math.floor(totalPrice),
          productImgUrl: this.selections[0].product.imageFile,
          ...this.form,
        };
